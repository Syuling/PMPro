...
class Adapter(nn.Module):
    def __init__(self, c_in, reduction=4, residual_ratio=0.2):
        ...
        # self.fc2 = LogitHead(nn.Linear(c_in, n_class, bias=False))

    def forward(self, x):
        a = self.fc(x)
        fea = self.residual_ratio * a + (1 - self.residual_ratio) * x
        # x = self.fc2(fea)
        return fea#,x
...
def make_classifier_head(classifier_head,
    ...
    if classifier_init == 'zeroshot':
        ...
        # linear_coarse_head.weight.data = get_zero_shot_weights(
        #         zeroshot_dataset, num_coarse_classes, in_features, text_encoder)
    
    if classifier_head == 'linear':
        head = linear_head
        # coarse_head = linear_coarse_head
    elif classifier_head == 'adapter':
        head = Adapter(in_features, residual_ratio=0.2)
        # head = nn.Sequential(
        #     adapter,
        #     linear_head
        # )
